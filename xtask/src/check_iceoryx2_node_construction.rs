...
//! Bans building an iceoryx2 node anywhere but the engine's one domain function
//! in `iceoryx2/node.rs`, and reading iceoryx2's global configuration anywhere at
//! all, that file included.
//!
...
/// The call that builds a node, allowed only in [`ALLOWED_FILE`].
...
pub fn run(workspace_root: &Path) -> Result<()> {
    ...
    let failure_lines: Vec<String> = report
        ...
        .map(|violation| {
            format!(
                "{}:{}: an iceoryx2 node built outside {ALLOWED_FILE}, or any read of iceoryx2's \
                 global configuration, lands outside the engine-owned domain. Use \
    ...
    tracing::info!(
        "check-iceoryx2-node-construction: {} files scanned across {SCAN_ROOTS:?}, no iceoryx2 \
         node built outside {ALLOWED_FILE} and no global configuration read",
        report.files_scanned,
    );
    Ok(())
}
...
fn collect_violations(relative_path: &Path, content: &str, violations: &mut Vec<Violation>) {
    let node_builder_is_allowed = relative_path == Path::new(ALLOWED_FILE);
    for (index, line) in content.lines().enumerate() {
        if line.trim_start().starts_with("//") {
            continue;
        }
        let builds_a_node_outside_the_allowed_file =
            !node_builder_is_allowed && line.contains(NODE_BUILDER_CALL);
        if builds_a_node_outside_the_allowed_file || line.contains(GLOBAL_CONFIG_CALL) {
