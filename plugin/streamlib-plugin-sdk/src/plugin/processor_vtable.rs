...
//! Per-processor [`ProcessorVTable`] construction (cdylib arm).
//!
...
//!
//! This is the cdylib-arm copy of the engine's `processor_vtable.rs`. The
//! vtable lives in the cdylib's binary, so its fn pointers target the
//! cdylib's address space; the host stores the pointer and dispatches
//! every host-called method through extern "C".
...
use crate::processors::{Config, GeneratedProcessor};

/// Map from `TypeId::of::<P>()` to the leaked `&'static ProcessorVTable`
/// for that P. One vtable per processor type in this cdylib's process
/// address space; rebuilt only on the first `vtable_for::<P>()` call.
static VTABLES: OnceLock<Mutex<HashMap<TypeId, &'static ProcessorVTable>>> = OnceLock::new();

/// Returns a `&'static ProcessorVTable` whose entries dispatch every
/// host-called method to `P`'s implementation. Looked up by `TypeId` and
/// cached per process.
    ...
    P::Config: Config,
{
    ...
    unsafe extern "C" fn set_iceoryx2_resources(
        ...
        run_host_extern_c(
            ...
            || {
                let processor = unsafe { &mut *(instance as *mut P) };
                // Reconstruct PluginAbiObjects from the (handle, vtable) pairs the
                // host hands us. Null handle/vtable = "this processor has no
                // outputs/inputs" — pass None to the trait method.
                let output_writer =
                    if output_writer_handle.is_null() || output_writer_vtable.is_null() {
                        None
                    } else {
                        Some(OutputWriter::from_raw_parts(
                ...
                let input_mailboxes =
                    if input_mailboxes_handle.is_null() || input_mailboxes_vtable.is_null() {
                        ...
                        Some(InputMailboxes::from_raw_parts(
