...
# every processor package in ./streamlib_modules/. Reverse the SDK link with
# `streamlib unlink --engine`.
set -euo pipefail
cd "$(dirname "$0")"
...
#    - this example's two example-local Python packages (each pins its own
#      conflicting numpy, proving per-package venv isolation).
"$STREAMLIB" link ./pkg-a/python
"$STREAMLIB" link ./pkg-b/python
