...
"""Per-package venv isolation proof — package B's version reporter.

Package B's pyproject pins ``numpy==2.1.3``. Its sibling package A pins
``numpy==1.26.4`` — a NumPy 1.x vs 2.x ABI conflict that could never
co-resolve in a single shared environment. This processor imports numpy
and reports ``numpy.__version__`` so the host can assert that package B's
