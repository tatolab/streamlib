...
#
# After it runs, `cargo run -- --output-dir=/tmp/skia-canvas-py` builds against
# the local SDK and the runtime finds every processor package in
...
#    - this example's Python polyglot processor package (Python is the only
...
echo "    cargo run -- --output-dir=/tmp/skia-canvas-py"
