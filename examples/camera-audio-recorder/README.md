...
Camera + microphone → synchronized MP4. **Deferred no-op at HEAD.**

Running the binary prints a deferral message and exits — there is no Linux path
yet. Synchronized audio+video MP4 recording is macOS/iOS-only (the Linux MP4
writer has no audio input), and the original pipeline still used the deprecated
compile-time typed-struct API. The real pipeline lives in git history;
