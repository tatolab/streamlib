...
Receive H.264 + Opus from a WHEP endpoint and play it. **Deferred no-op at
HEAD.**

Running the binary prints a deferral message and exits. The player decodes the
received H.264 with VideoToolbox (macOS hardware decode) and is macOS-only as
written; it also uses the deprecated compile-time typed-struct API. A Linux
version — `WebRtcWhepProcessor → @tatolab/h264` (Vulkan) decoder →
`@tatolab/display` — is future work. The real pipeline lives in git history;
`src/main.rs` carries the deferral note and the path back.
