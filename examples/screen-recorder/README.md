...
Screen capture → MP4. **Deferred no-op at HEAD.**

Running the binary prints a deferral message and exits. Screen capture is
macOS/iOS-only (`@tatolab/screen-capture` via ScreenCaptureKit) — there is no
Linux screen-capture backend yet — and the original pipeline still used the
deprecated compile-time typed-struct API. The real pipeline lives in git
history; `src/main.rs` carries the deferral note and the path back.
