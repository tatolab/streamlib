...
Plays a C major chord: `@tatolab/audio`'s `ChordGenerator` → `AudioOutput`.
...
```rust
let chord_gen_ident = processor_type_ref!("tatolab", "audio", "ChordGenerator"); // no version, no load call
...
- The referenced processor's package lives in this app's **`streamlib_modules/`**
  ...
  the runtime discovers `@tatolab/audio` there and lazily loads it.
...
2. **Packages** — `streamlib link` symlinks `@tatolab/audio` from the in-repo
   `packages/` into `./streamlib_modules/`. Edits in that package are live on
   the next run.
