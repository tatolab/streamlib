...
#
# This is a pure engine/RHI demo — it loads no processor packages, so setup only
# links the SDK so `cargo run` builds against the local checkout. Reverse it
# with: streamlib unlink --engine
set -euo pipefail
cd "$(dirname "$0")"
