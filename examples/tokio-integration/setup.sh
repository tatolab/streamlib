...
#
# This demo drives `Runner` directly and loads no processor packages, so setup
# only links the SDK so `cargo run` builds against the local checkout. Reverse
# it with: streamlib unlink --engine
set -euo pipefail
cd "$(dirname "$0")"
...
# SDK: point this app's `streamlib` dep at the local checkout via a
