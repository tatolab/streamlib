...
# orchestrator's build of each linked package — AND this example's own build.rs
# jtd-codegen — resolves its schema deps via STREAMLIB_LINK_CHECKOUT, a separate
# build-time pointer. Publish it to every `cargo build`/`cargo run` in this dir
# through the .cargo [env] table so `./setup.sh && cargo run` is genuinely one
# step.
...
"$STREAMLIB" link "$CHECKOUT/packages/audio"
"$STREAMLIB" link "$CHECKOUT/packages/camera"
"$STREAMLIB" link "$CHECKOUT/packages/h264"
"$STREAMLIB" link "$CHECKOUT/packages/opus"
"$STREAMLIB" link "$CHECKOUT/packages/webrtc"
