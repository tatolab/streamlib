...
# 2. Packages into ./streamlib_modules/: this example's own counting-sink
#    plugin, Python source, and Deno source polyglot packages.
...
echo "    cargo run -- --runtime=python"
echo "    cargo run -- --runtime=deno"
