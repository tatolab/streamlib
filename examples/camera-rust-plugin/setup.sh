...
# `link --engine` writes the cargo [patch.crates-io], but the build-time schema
...
# it to every `cargo build`/`cargo run` in this dir through the .cargo [env]
# table so `./setup.sh && cargo run` is genuinely one step.
