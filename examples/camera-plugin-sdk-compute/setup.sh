...
# every processor package (including the example-local ./plugin) in
...
# 1. SDK: point this app's `streamlib` deps at the local checkout.
"$STREAMLIB" link --engine "$CHECKOUT"

# `link --engine` writes the cargo [patch.crates-io], but the plugin's build-time
# schema codegen (and the runtime orchestrator's package builds) resolve schema
# deps via STREAMLIB_LINK_CHECKOUT. Publish it through the .cargo [env] table so
# `./setup.sh && cargo run` is one step.
