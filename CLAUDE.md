...
- **`streamlib::vulkan::rhi`** (in `libs/streamlib-engine/src/vulkan/rhi/`) — the host-side RHI. Owns `HostVulkanDevice` (allocator + queue matrix + swapchain extensions), `HostVulkanTexture`, `HostVulkanPixelBuffer`, `HostVulkanTimelineSemaphore`, `VulkanComputeKernel`, the modifier probe, etc.
