...
**Where the dream currently leaks**: a handful of `GpuContextFullAccess` vtable slots still transit an `Arc<HostInternalType>` by raw `Arc::into_raw` / `Arc::from_raw` pointer, and those host-internal types are **not** `#[repr(C)]`, so a separately-built plugin can lay them out differently and read them at the wrong offsets. The residual transit set is exactly three engine types — `HostVulkanDevice` (`host_vulkan_device_arc`), `HostVulkanTexture` (`host_vulkan_texture_arc`), and `HostVulkanTimelineSemaphore` (the timeline-semaphore create/set/wait/get slots). A facade plugin (Cargo-deps `streamlib`, statically-linking the engine) is the flavor that can reach these slots; an engine-free plugin carries no such surface.

Since 2026-07-12, **a mixed host/plugin build is refused at load rather than silently corrupting the driver**: `PluginDeclaration` (ABI v5) carries an `engine_transit_fingerprint` folding the engine version, `streamlib-consumer-rhi` version, and the first-order layout of those three transit types, and `validate_plugin_declaration` rejects a mismatch with a typed `Error::PluginBuildMismatch` naming both build identities. That closes the *silent-corruption* failure mode (see @docs/architecture/plugin-abi.md → "Load handshake" and @docs/learnings/slpkg-raw-device-rhi-construction.md). It does **not** make the transit *safe* — a build that matches the fingerprint but reordered a transit type's fields at identical size (which `repr(Rust)` permits) would still load. The fully sound fix remains the PluginAbiObject lift of those slots into `#[repr(C)] { handle, vtable }` pairs (mirroring `RhiCommandQueue` / `CommandBuffer`); until that lands, the fingerprint is the guard.

> ~~Issue #917 closes that gap ... After #917 lands, no plugin ABI return type leaks host layout, and the cross-repo dream is fully real.~~ — Superseded 2026-07-12. #917 landed the `#[repr(C)]` PluginAbiObject pairs for most return types, but the three raw-`Arc` FullAccess slots above shipped after / outside it, so the transit surface is not empty. The build-fingerprint handshake makes mixed builds *fail loud at load*; the raw-`Arc` slots' PluginAbiObject lift is the remaining work that would make the dream fully real.
