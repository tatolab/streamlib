...
//!
//! The camera's AVFoundation callback receives `CVPixelBuffer` handles
//! and walks them down to the backing `IOSurface` so the engine's
//! `GpuContext::blit_copy_iosurface` path can stage the frame into a
//! pooled `PixelBuffer`. These six declarations are everything the
//! camera processor uses — duplicated here rather than shared via SDK
//! pollution.
