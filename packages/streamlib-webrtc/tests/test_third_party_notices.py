...
# shape: the WebRTC stack itself, two of the media crates it brings with it, the
# HTTP client the signalling rides, the TLS layer under both, and the binding
# layer this wheel is built on. Enough to catch a notices file generated against
# the wrong manifest; not so many that a dependency swap fails an unrelated
# test. Matched in the bracketed link form the generator writes, so a name
