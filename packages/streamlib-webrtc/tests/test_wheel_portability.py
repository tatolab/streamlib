...
def test_the_native_extension_links_nothing_the_host_may_not_supply():
    ...
    assert set(needed) <= LIBRARIES_THE_HOST_MAY_SUPPLY, (
        ...
        "installed on the user's machine for `import streamlib_webrtc` to work"
    )


def test_the_tls_stack_is_inside_the_artifact():
    """rustls and its `ring` backend are the reason to check: an OpenSSL-backed
    TLS stack would put `libssl` and `libcrypto` on this list, and their ABI
    changes under a wheel that was built against another version."""
    needed = dynamic_libraries_needed_by(the_native_extension())
