...
second call site is the one that matters here: a session runs in the helper, so
the stack it needs has to be up in the helper.
"""
...
def load(host: CapabilityExtensionHost) -> None:
    ...
    host.register_capability(
        CAPABILITY_NAME, importlib.metadata.version("streamlib-webrtc")
    )
