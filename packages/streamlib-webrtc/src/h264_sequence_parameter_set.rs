...
mod tests {
    ...
    /// Writes the syntax elements an SPS is made of, so a test can state a
    /// stream's shape rather than a hex blob nobody can check.
    #[derive(Default)]
    struct RawBitstreamWriter {
        bytes: Vec<u8>,
    ...
    /// 320x180 4:2:0 progressive: 20 macroblocks across, 12 map units down
    /// (192 coded), the bottom 12 rows cropped away.
    fn baseline_320x180(vui: impl FnOnce(&mut RawBitstreamWriter)) -> Vec<u8> {
    ...
    fn no_vui(writer: &mut RawBitstreamWriter) {
        writer.bit(0);
    }
