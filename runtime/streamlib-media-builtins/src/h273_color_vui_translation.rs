...
pub fn primaries_from_h273_byte(byte: u8) -> Option<Primaries> {
    Some(match byte {
        _ if byte == primaries::BT709 => Primaries::Bt709,
        _ if byte == primaries::BT470_M => Primaries::Bt470M,
        _ if byte == primaries::BT470_BG => Primaries::Bt470Bg,
        _ if byte == primaries::SMPTE170M => Primaries::Smpte170m,
        _ if byte == primaries::SMPTE240M => Primaries::Smpte240m,
        _ if byte == primaries::FILM => Primaries::Film,
        _ if byte == primaries::BT2020 => Primaries::Bt2020,
        _ if byte == primaries::SMPTE428 => Primaries::Smpte428,
        _ if byte == primaries::SMPTE431 => Primaries::Smpte431,
        _ if byte == primaries::SMPTE432 => Primaries::Smpte432,
        _ if byte == primaries::EBU3213 => Primaries::Ebu3213,
        _ => return None,
    })
}
...
pub fn transfer_from_h273_byte(byte: u8) -> Option<Transfer> {
    Some(match byte {
        _ if byte == transfer::BT709 => Transfer::Bt709,
        _ if byte == transfer::GAMMA22 => Transfer::Gamma22,
        _ if byte == transfer::GAMMA28 => Transfer::Gamma28,
        _ if byte == transfer::SMPTE170M => Transfer::Smpte170m,
        _ if byte == transfer::SMPTE240M => Transfer::Smpte240m,
        _ if byte == transfer::LINEAR => Transfer::Linear,
        _ if byte == transfer::LOG100 => Transfer::Log100,
        _ if byte == transfer::LOG100_SQRT10 => Transfer::Log100Sqrt10,
        _ if byte == transfer::XVYCC => Transfer::Xvycc,
        _ if byte == transfer::BT1361 => Transfer::Bt1361,
        _ if byte == transfer::SRGB => Transfer::Srgb,
        _ if byte == transfer::BT2020_TEN_BIT => Transfer::Bt2020TenBit,
        _ if byte == transfer::BT2020_TWELVE_BIT => Transfer::Bt2020TwelveBit,
        _ if byte == transfer::SMPTE2084 => Transfer::Smpte2084,
        _ if byte == transfer::SMPTE428 => Transfer::Smpte428,
        _ if byte == transfer::ARIB_STD_B67 => Transfer::AribStdB67,
        _ => return None,
    })
}
...
pub fn matrix_from_h273_byte(byte: u8) -> Option<Matrix> {
    Some(match byte {
        _ if byte == matrix::IDENTITY => Matrix::Identity,
        _ if byte == matrix::BT709 => Matrix::Bt709,
        _ if byte == matrix::FCC => Matrix::Fcc,
        _ if byte == matrix::BT470_BG => Matrix::Bt470Bg,
        _ if byte == matrix::SMPTE170M => Matrix::Smpte170m,
        _ if byte == matrix::SMPTE240M => Matrix::Smpte240m,
        _ if byte == matrix::YCGCO => Matrix::Ycgco,
        _ if byte == matrix::BT2020_NCL => Matrix::Bt2020Ncl,
        _ if byte == matrix::BT2020_CL => Matrix::Bt2020Cl,
        _ if byte == matrix::SMPTE2085 => Matrix::Smpte2085,
        _ if byte == matrix::CHROMA_NCL => Matrix::ChromaNcl,
        _ if byte == matrix::CHROMA_CL => Matrix::ChromaCl,
        _ if byte == matrix::ICTCP => Matrix::Ictcp,
        _ => return None,
    })
}
