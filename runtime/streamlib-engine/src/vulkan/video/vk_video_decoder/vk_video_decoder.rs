...
use vulkanalia::vk::{
    self, KhrVideoDecodeQueueExtensionDeviceCommands, KhrVideoQueueExtensionDeviceCommands,
};
...
use crate::vulkan::rhi::{
    HostVulkanBuffer, HostVulkanTexture, HostVulkanVideoSession, HostVulkanVideoSessionParameters,
    VideoBitstreamBufferDescriptor, VideoBitstreamDirection, VideoDpbDirection,
...
pub struct VkVideoDecoder {
    ...
    ctx: Arc<crate::vulkan::video::video_context::VideoContext>,
    queue: vk::Queue,
...
impl VkVideoDecoder {
    ...
    pub fn new(
        ...
        if codec_operation == vk::VideoCodecOperationFlagsKHR::DECODE_H264 {
            video_profile.next = &*h264_profile as *const vk::VideoDecodeH264ProfileInfoKHR
                as *const std::ffi::c_void;
        } else if codec_operation == vk::VideoCodecOperationFlagsKHR::DECODE_H265 {
            video_profile.next = &*h265_profile as *const vk::VideoDecodeH265ProfileInfoKHR
                as *const std::ffi::c_void;
        }
    ...
    pub fn start_video_sequence(&mut self, video_format: &VkParserDetectedVideoFormat) -> i32 {
        ...
        let video_session =
            match HostVulkanVideoSession::new(self.ctx.host_device(), &session_descriptor) {
                Ok(s) => s,
                Err(e) => {
        ...
        let dpb_texture =
            match HostVulkanTexture::new_video_dpb(self.ctx.host_device(), &dpb_descriptor) {
                Ok(t) => t,
                Err(e) => {
        ...
        let mut ycbcr_info =
            vk::SamplerYcbcrConversionInfo::builder().conversion(self.ctx.nv12_ycbcr_conversion());
        ...
        match HostVulkanBuffer::new_video_bitstream(self.ctx.host_device(), &bs_descriptor) {
            Ok(buffer) => {
    ...
    pub unsafe fn wait_for_decode(&mut self) -> VideoResult<()> {
        unsafe {
            if self.decode_in_flight {
                let device = self.ctx.device();
                device
                    .wait_for_fences(&[self.fence], true, LONG_TIMEOUT)
                    .map_err(VideoError::from)?;
    ...
    pub unsafe fn decode_frame(
        ...
        unsafe {
            ...
            if aligned_size > self.bitstream_size {
                ...
                .map_err(|e| VideoError::BitstreamError(format!("bitstream resize: {e}")))?;
            ...
            if padding > 0 {
                ptr::write_bytes(bitstream_mapped_ptr.add(submit.bitstream.len()), 0, padding);
            }
            ...
            if let Some(ref h264) = submit.h264_info {
                let mut flags: vk::video::StdVideoDecodeH264PictureInfoFlags = std::mem::zeroed();
                if h264.is_idr {
                ...
                if h264.is_reference {
                    flags.set_is_reference(1);
                }
                if h264.field_pic_flag {
                    flags.set_field_pic_flag(1);
                }
                if h264.bottom_field_flag {
                ...
                for ref_info in &h264.ref_pic_infos {
                    let mut rf: vk::video::StdVideoDecodeH264ReferenceInfoFlags =
                        std::mem::zeroed();
                    if ref_info.long_term_ref {
                        rf.set_used_for_long_term_reference(1);
                    }
                    if ref_info.non_existing {
                        rf.set_is_non_existing(1);
                    }
            ...
            if let Some(ref h265) = submit.h265_info {
                ...
                if !h265.is_idr && !h265.ref_pic_infos.is_empty() {
                    ...
                    for i in 0..nb.min(8) {
                        ref_pic_set_st_curr_before[i] = submit.reference_slots[i].slot_index as u8;
                    }
                    for i in 0..na.min(8) {
                ...
                for ref_info in &h265.ref_pic_infos {
                    let mut rf: vk::video::StdVideoDecodeH265ReferenceInfoFlags =
                        std::mem::zeroed();
                    if ref_info.long_term_ref {
                        rf.set_used_for_long_term_reference(1);
                    }
            ...
            let submit_info = vk::SubmitInfo2::builder()
                ...
                .build();
            self.host_device
                .submit_to_queue(self.queue, &[submit_info], self.fence)
                .map_err(VideoError::from)?;
    ...
    pub fn set_session_parameters_arc(&mut self, params: Arc<HostVulkanVideoSessionParameters>) {
...
mod tests {
    ...
    fn test_decoder_features_flags() {
        let features =
            DecoderFeatures::ENABLE_LINEAR_OUTPUT | DecoderFeatures::ENABLE_HW_LOAD_BALANCING;
