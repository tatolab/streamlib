...
use crate::vulkan::video::vk_video_encoder::vk_encoder_dpb_h264::PicInfoH264;
use crate::vulkan::video::vk_video_encoder::vk_encoder_dpb_h265::{
    MAX_NUM_LIST_REF_H265, NO_REFERENCE_PICTURE_H265, RefPicSetH265, SetupRefListResult,
};
...
impl SimpleEncoder {
    ...
    pub(crate) unsafe fn encode_frame(
        ...
        unsafe {
            ...
            tracing::debug!(pts = pts, frame_type = frame_type.name(), "Encoding frame");
            ...
            let h265_rps_result: Option<
                crate::vulkan::video::vk_video_encoder::vk_encoder_dpb_h265::InitializeRpsResult,
            >;
            ...
            if is_h265 && self.h265_encoder.is_some() {
                ...
                tracing::debug!(
                    h265_poc = h265_poc,
                    pic_type = pic_type,
                    "H265 encode Step 1: reference_picture_marking"
                );
                ...
                tracing::debug!(slot_idx = slot_idx, "  Step 3 result: allocated DPB slot");

                // Step 4: SetupReferencePictureListLx (build RefPicList0/1)
                tracing::debug!(
                    pic_type = pic_type,
                    "H265 encode Step 4: setup_reference_picture_list_lx"
                );
                let ref_list_res = if pic_type == 2 || pic_type == 3 {
                    // P or B frame
                    Some(dpb.setup_reference_picture_list_lx(pic_type, &rps, 1, 0))
                } else {
                    None
                };
                ...
                tracing::debug!(
                    ...
                    "H265 encode Step 5: dpb_picture_end"
                );
            ...
            let h264_setup_ref_info = vk::video::StdVideoEncodeH264ReferenceInfo {
                ...
                primary_pic_type: match frame_type {
                    FrameType::Idr | FrameType::I => vk::video::STD_VIDEO_H264_PICTURE_TYPE_IDR,
            ...
            for &ref_idx in &reference_slot_indices {
                ...
                if self.codec_flag == vk::VideoCodecOperationFlagsKHR::ENCODE_H264 {
                    ...
                    h264_ref_dpb_infos.push(vk::video::StdVideoEncodeH264ReferenceInfo {
                        ...
                        PicOrderCnt: ref_poc,
                        long_term_pic_num: 0,
                        long_term_frame_idx: if ref_is_long_term {
                            ref_long_term_idx as u16
                        } else {
                            0
                        },
                        temporal_id: 0,
                    ...
                    // Use DPB-provided reference info when available
                    let (ref_poc, ref_tid) = if let Some(ref enc) = self.h265_encoder {
                        let mut poc = 0u32;
                        let mut tid = 0i32;
                        let mut _unused = false;
                        enc.dpb.fill_std_reference_info(
                            ref_idx as u8,
                            &mut poc,
                            &mut tid,
                            &mut _unused,
            ...
            if self.codec_flag == vk::VideoCodecOperationFlagsKHR::ENCODE_H264 {
                begin_setup_slot.next =
                    &mut h264_begin_setup_dpb as *mut _ as *const std::ffi::c_void;
            } else if self.codec_flag == vk::VideoCodecOperationFlagsKHR::ENCODE_H265 {
                begin_setup_slot.next =
                    &mut h265_begin_setup_dpb as *mut _ as *const std::ffi::c_void;
            }
            ...
            if is_h265 {
                ...
                for (i, slot) in all_slots.iter().enumerate() {
                    tracing::debug!(idx = i, slot_index = slot.slot_index, "  all_slots entry");
                }
            }
            ...
            let mut begin_rc_info =
                vk::VideoEncodeRateControlInfoKHR::builder().rate_control_mode(rc_mode);

            if needs_layers {
                begin_rc_info = begin_rc_info.layers(std::slice::from_ref(&rate_control_layer));
            }
            ...
            if !self.rate_control_sent {
                let mut control_rc_info =
                    vk::VideoEncodeRateControlInfoKHR::builder().rate_control_mode(rc_mode);

                if needs_layers {
                    control_rc_info =
                        control_rc_info.layers(std::slice::from_ref(&rate_control_layer));
                }
            ...
            if self.codec_flag == vk::VideoCodecOperationFlagsKHR::ENCODE_H264 {
                ...
                let h264_slice_type = match frame_type {
                    FrameType::Idr | FrameType::I => vk::video::STD_VIDEO_H264_SLICE_TYPE_I,
                    FrameType::P => vk::video::STD_VIDEO_H264_SLICE_TYPE_P,
                    FrameType::B => vk::video::STD_VIDEO_H264_SLICE_TYPE_B,
                };
                ...
                h264_slice_header = vk::video::StdVideoEncodeH264SliceHeader {
                    ...
                    slice_qp_delta: 0,
                    reserved1: 0,
                    cabac_init_idc: vk::video::STD_VIDEO_H264_CABAC_INIT_IDC_0,
                ...
                let h265_slice_type = match frame_type {
                    FrameType::Idr | FrameType::I => vk::video::STD_VIDEO_H265_SLICE_TYPE_I,
                    FrameType::P => vk::video::STD_VIDEO_H265_SLICE_TYPE_P,
                    FrameType::B => vk::video::STD_VIDEO_H265_SLICE_TYPE_B,
                };
                ...
                let mut std_flags: vk::video::StdVideoEncodeH265PictureInfoFlags =
                    std::mem::zeroed();
                std_flags.set_is_reference(if frame_type != FrameType::B { 1 } else { 0 });
                std_flags.set_IrapPicFlag(if is_idr || frame_type == FrameType::I {
                    1
                } else {
                    0
                });
                ...
                let ref_lists_ptr: *const vk::video::StdVideoEncodeH265ReferenceListsInfo =
                    if let Some(ref rl) = h265_ref_list_result {
                ...
                if let Some(ref rps_res) = h265_rps_result {
                    ...
                    h265_short_term_ref_pic_set =
                        std::mem::zeroed::<vk::video::StdVideoH265ShortTermRefPicSet>();
                }
            ...
            if size > 0 && !bitstream_mapped_ptr.is_null() {
                ptr::copy_nonoverlapping(bitstream_mapped_ptr.add(offset), data.as_mut_ptr(), size);
            }
            ...
            if is_h265 && self.h265_encoder.is_some() {
                ...
                self.dpb_slots[setup_slot_index].frame_num = self.frame_count;
                self.dpb_slots[setup_slot_index].poc =
