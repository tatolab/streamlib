...
use crate::vulkan::video::vk_video_encoder::vk_encoder_config_h264::{
    EncoderConfigH264, level_index_to_std_video, profile_idc_to_std_video,
};
...
/// The `chroma_sample_loc_type_*_field` value this encoder writes into every SPS VUI
/// it emits, for both codecs.
///
/// Bitstream contract: the emitted VUI never sets `chroma_loc_info_present_flag`, so
/// H.264 §E.2.1 / H.265 §E.3.1 have decoders infer this same type anyway — the written
/// value and the inferred one agree, and both describe what `rgb_to_nv12.comp`
/// produces. The decode side reconstructs at the siting this names; changing it here
/// without moving `nv12_to_rgb`'s offsets shifts chroma against the wire.
pub(crate) const EMITTED_CHROMA_SAMPLE_LOC_TYPE: u8 = 0;
...
impl SimpleEncoder {
    ...
    pub(crate) unsafe fn configure(&mut self, config: &EncodeConfig) -> VideoResult<()> {
        unsafe {
            ...
            let h264_cfg = if self.codec_flag == vk::VideoCodecOperationFlagsKHR::ENCODE_H264 {
                ...
                cfg.base
                    .gop_structure
                    .set_consecutive_b_frame_count(config.num_b_frames);
            ...
            let mut h265_profile =
                vk::VideoEncodeH265ProfileInfoKHR::builder().std_profile_idc(h265_profile_idc);
            ...
            let _profile_list = vk::VideoProfileListInfoKHR::builder()
            ...
            let h265_encode_caps_ptr: *const vk::VideoEncodeH265CapabilitiesKHR = &h265_encode_caps;
            ...
            let granularity_w = caps
                .picture_access_granularity
                .width
                .max(H264_MB_SIZE_ALIGNMENT);
            let granularity_h = caps
                .picture_access_granularity
                .height
                .max(H264_MB_SIZE_ALIGNMENT);
            ...
            let bitstream_buffer_owner = self.create_bitstream_buffer(bs_size, &profile_info)?;
    ...
    pub(crate) unsafe fn extract_header(&self) -> VideoResult<Vec<u8>> {
        unsafe {
            ...
            let data =
                device.get_encoded_video_session_parameters_khr(&get_info, Some(&mut feedback))?;
    ...
    unsafe fn create_session_parameters(
        ...
        aligned_h: u32,
        quality_level: u32,
    ) -> VideoResult<Arc<crate::vulkan::rhi::HostVulkanVideoSessionParameters>> {
        unsafe {
            ...
            if self.codec_flag == vk::VideoCodecOperationFlagsKHR::ENCODE_H264 {
                ...
                h264_sps_vui = vk::video::StdVideoH264SequenceParameterSetVui {
                    ...
                    chroma_sample_loc_type_top_field: EMITTED_CHROMA_SAMPLE_LOC_TYPE,
                    chroma_sample_loc_type_bottom_field: EMITTED_CHROMA_SAMPLE_LOC_TYPE,
                    reserved1: 0,
                    pHrdParameters: ptr::null(),
                };

                let mut sps_flags: vk::video::StdVideoH264SpsFlags = std::mem::zeroed();
                sps_flags.set_direct_8x8_inference_flag(
                    if state.sps_info.direct_8x8_inference_flag {
                        1
                    } else {
                        0
                    },
                );
                sps_flags.set_frame_mbs_only_flag(if state.sps_info.frame_mbs_only_flag {
                    1
                } else {
                    0
                });
                sps_flags.set_frame_cropping_flag(if state.sps_info.frame_cropping_flag {
                    1
                } else {
                    0
                });
                ...
                h264_sps = vk::video::StdVideoH264SequenceParameterSet {
                    ...
                    chroma_format_idc: vk::video::STD_VIDEO_H264_CHROMA_FORMAT_IDC_420,
                    ...
                    pic_order_cnt_type: vk::video::STD_VIDEO_H264_POC_TYPE_0,
                    offset_for_non_ref_pic: 0,
                    offset_for_top_to_bottom_field: 0,
                    log2_max_pic_order_cnt_lsb_minus4: state
                        .sps_info
                ...
                pps_flags.set_transform_8x8_mode_flag(if state.pps_info.transform_8x8_mode_flag {
                    1
                } else {
                    0
                });
                pps_flags.set_deblocking_filter_control_present_flag(
                    if state.pps_info.deblocking_filter_control_present_flag {
                        1
                    } else {
                        0
                    },
                );
                pps_flags.set_entropy_coding_mode_flag(
                    if state.pps_info.entropy_coding_mode_flag {
                        1
                    } else {
                        0
                    },
                ...
                h264_pps = vk::video::StdVideoH264PictureParameterSet {
                    ...
                    num_ref_idx_l0_default_active_minus1: state
                        .pps_info
                        .num_ref_idx_l0_default_active_minus1,
                    num_ref_idx_l1_default_active_minus1: state
                        .pps_info
                ...
                h265_sps = vk::video::StdVideoH265SequenceParameterSet {
                    flags: sps_flags,
                    chroma_format_idc: vk::video::STD_VIDEO_H265_CHROMA_FORMAT_IDC_420,
            ...
            let descriptor = crate::vulkan::rhi::VideoSessionParametersDescriptor {
                ...
                quality_level: if quality_level > 0 {
                    Some(quality_level)
                } else {
                    None
                },
    ...
    unsafe fn create_dpb_images(
        ...
        unsafe {
            ...
            use crate::vulkan::rhi::{
                HostVulkanTexture, VideoDpbDirection, VideoDpbTextureDescriptor,
            };
            ...
            if use_separate_images {
                ...
                for i in 0..count {
                    ...
                    let texture =
                        HostVulkanTexture::new_video_dpb(self.ctx.host_device(), &descriptor)
                            .map_err(|e| {
                                VideoError::BitstreamError(format!("encoder dpb separate: {e}"))
                            })?;
                    let image = texture.image().expect(
                        "HostVulkanTexture::new_video_dpb returns a texture with vk::Image",
                    );
                ...
                let texture = HostVulkanTexture::new_video_dpb(self.ctx.host_device(), &descriptor)
                    .map_err(|e| VideoError::BitstreamError(format!("encoder dpb shared: {e}")))?;
                let image = texture
                    .image()
    ...
    unsafe fn create_bitstream_buffer(
        ...
        use crate::vulkan::rhi::{
            HostVulkanBuffer, VideoBitstreamBufferDescriptor, VideoBitstreamDirection,
        };
        ...
        HostVulkanBuffer::new_video_bitstream(self.ctx.host_device(), &descriptor)
            .map_err(|e| VideoError::BitstreamError(format!("encoder bitstream: {e}")))
    }
}
