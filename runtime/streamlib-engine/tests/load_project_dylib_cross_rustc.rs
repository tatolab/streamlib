...
//! `tools/streamlib-cross-rustc-fixture/` is a standalone Cargo
...
fn dlopen_cross_rustc_fixture_round_trips_every_plugin_abi_object() {
    ...
    let fixture_src = workspace_root.join("tools/streamlib-cross-rustc-fixture");
    ...
    // (`tools/streamlib-cross-rustc-fixture/` + `packages/core/`) inside
    // the tempdir so the fixture's `streamlib.yaml` patch entry
    // `path: ../../packages/core` resolves.
    let tmp = tempfile::tempdir().unwrap();
    let fixture_dst = tmp.path().join("tools/streamlib-cross-rustc-fixture");
