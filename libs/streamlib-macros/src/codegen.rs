...
///   `@tatolab/audio/AudioMixerConfig@1.0.0`) — yields `AudioMixerConfig`
///   verbatim from the segment after the last `/`.
/// - Legacy reverse-DNS `com.<org>.<processor>.config@<version>` (e.g.
///   `com.tatolab.buffer_rechunker.config@1.0.0`) — yields
///   `BufferRechunkerConfig` from the segment before the trailing `.config`.
///
/// The actual type must be defined by the user (or generated by codegen) and
/// match this name in the package's `_generated_` module.
fn derive_config_type_from_schema(schema_ref: &str) -> TokenStream {
    let pascal_name = if let Some(rest) = schema_ref.strip_prefix('@') {
        ...
        format!("{}Config", to_pascal_case(processor_segment))
    };
