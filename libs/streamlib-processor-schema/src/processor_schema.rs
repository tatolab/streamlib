...
/// Role hint for a processor's dedicated thread.
///
/// Used as the default OS-thread-name prefix (`{kind}-{id}`, lowercase) so
/// `/proc/<pid>/task/*/comm` and tracing-by-thread lookups can group
/// processors of the same kind. When absent the thread defaults to
/// `processor-{id}`.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum ProcessorSchedulingKind {
    Camera,
    Display,
    Audio,
    Video,
...
impl ProcessorSchedulingKind {
    /// Lowercase prefix used in the default `{kind}-{id}` thread-name template.
    pub fn thread_name_prefix(&self) -> &'static str {
        match self {
            ProcessorSchedulingKind::Camera => "camera",
            ProcessorSchedulingKind::Display => "display",
            ProcessorSchedulingKind::Audio => "audio",
            ProcessorSchedulingKind::Video => "video",
            ProcessorSchedulingKind::Compositor => "compositor",
        }
    }
}
...
///
/// Optional; omission means [`ThreadPriority::Normal`] with the default
...
pub struct ProcessorScheduling {
    ...
    /// Optional role hint. Drives the default `{kind}-{id}` thread name; the
    /// value is also surfaced on the descriptor for downstream tooling
    /// (htop, perf, tracing).
    #[serde(default)]
    pub kind: Option<ProcessorSchedulingKind>,

    /// Explicit thread-name override. When present this string is used
    /// verbatim and takes precedence over `kind`'s default. Linux's
    /// `pthread_setname_np` truncates at 15 chars — keep it short.
    #[serde(default)]
