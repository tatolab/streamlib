...
impl crate::core::ReactiveProcessor for H265EncoderProcessor::Processor {
    async fn setup(&mut self, ctx: RuntimeContext) -> Result<()> {
        ...
        tracing::info!(
            "[H265Encoder] Initialized ({}x{}, aligned {}x{}, Vulkan Video hardware)",
    ...
    fn process(&mut self) -> Result<()> {
        ...
        let packets = unsafe {
            upload_and_encode(encoder, bgra_data, upload, src_width, src_height, timestamp_ns)?
        };
...
unsafe fn upload_and_encode(
    ...
    encoder.encode_image(upload.bgra_view, timestamp_ns)
        .map_err(|e| StreamError::Runtime(format!("H.265 encode failed: {e}")))
}
