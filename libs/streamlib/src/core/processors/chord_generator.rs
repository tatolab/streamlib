...
pub struct ChordGeneratorProcessor {
    ...
    // Initialized with Default::default(), real values set in on_start()
...
// SAFETY: JoinHandle<()> is Send, and all other fields are wrapped in Send types (Arc, Mutex, AtomicBool)
        ...
        let handle = std::thread::spawn(move || {
            ...
            while running.load(Ordering::Relaxed) {
                ...
                if iteration_count % 100 == 0 {
                    tracing::debug!("ChordGenerator iteration {}: Writing stereo chord frame", iteration_count);
                }
