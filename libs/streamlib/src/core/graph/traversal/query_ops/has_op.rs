use crate::core::graph::{Component, GraphEdge, GraphNode, LinkTraversal, LinkTraversalMut, ProcessorTraversal, ProcessorTraversalMut};
...
impl<'a> ProcessorTraversalMut<'a> {
    pub fn has<C: Component>(self) -> ProcessorTraversalMut<'a> {
        let new_ids = self
            .ids
            .iter()
            .filter_map(|&idx| self.graph.node_weight(idx).map(|node| (idx, node)))
...
impl<'a> LinkTraversalMut<'a> {
    pub fn has<C: Component>(self) -> LinkTraversalMut<'a> {
        let new_ids = self
            .ids
            .iter()
            .filter_map(|&idx| self.graph.edge_weight(idx).map(|link| (idx, link)))
