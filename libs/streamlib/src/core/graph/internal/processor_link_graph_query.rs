...
pub(crate) trait InternalProcessorLinkGraphQueryOperations {
    ...
    fn query_outgoing_link_ids(&self, id: impl AsRef<str>) -> Vec<LinkUniqueId>;

    /// Get IDs of incoming links to a processor.
    fn query_incoming_link_ids(&self, id: impl AsRef<str>) -> Vec<LinkUniqueId>;

    /// Get all link IDs in the graph.
    fn query_all_link_ids(&self) -> Vec<LinkUniqueId>;
}
