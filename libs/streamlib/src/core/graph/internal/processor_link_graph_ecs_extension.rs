...
pub(crate) struct InternalProcessorLinkGraphEcsExtension {
    ...
    /// Map from LinkUniqueId to hecs Entity.
    link_entities: HashMap<LinkUniqueId, Entity>,
}
...
impl InternalProcessorLinkGraphEcsExtension {
    ...
    pub(crate) fn ensure_link_entity(&mut self, id: &LinkUniqueId) -> Entity {
    ...
    pub(crate) fn get_link_entity(&self, id: &LinkUniqueId) -> Option<Entity> {
        self.link_entities.get(id).copied()
    }

    /// Remove a link's entity from the ECS world.
    pub(crate) fn remove_link_entity(&mut self, id: &LinkUniqueId) -> Option<Entity> {
    ...
    pub(crate) fn remove_link_component<C: Component>(&mut self, id: &LinkUniqueId) -> Result<()> {
        let entity = *self
            .link_entities
            .get(id)
            .ok_or_else(|| StreamError::NotFound(format!("Link '{}' not found", id)))?;
    ...
    pub(crate) fn get_link_component<C: Component>(&self, id: &LinkUniqueId) -> Option<hecs::Ref<'_, C>> {
        let entity = self.link_entities.get(id)?;
        self.world.get::<&C>(*entity).ok()
    }

    /// Get the state of a link from its ECS component.
    pub(crate) fn get_link_state(&self, id: &LinkUniqueId) -> Option<LinkState> {
        let entity = self.link_entities.get(id)?;
        self.world
    ...
    pub(crate) fn set_link_state(&mut self, id: &LinkUniqueId, state: LinkState) -> Result<()> {
        let entity = *self
            .link_entities
            .get(id)
            .ok_or_else(|| StreamError::NotFound(format!("Link '{}' not found", id)))?;
    ...
    pub(crate) fn link_entities(&self) -> &HashMap<LinkUniqueId, Entity> {
        &self.link_entities
    }
}
