...
pub trait GraphQueryInterface: FieldResolver {
    ...
    fn all_link_ids(&self) -> Vec<LinkUniqueId>;

    /// Get a link by ID.
    fn get_link(&self, id: &LinkUniqueId) -> Option<&Link>;

    /// Check if a link exists.
    fn has_link(&self, id: &LinkUniqueId) -> bool;

    /// Get the source processor of a link.
    fn get_link_source(&self, id: &LinkUniqueId) -> Option<ProcessorId>;

    /// Get the target processor of a link.
    fn get_link_target(&self, id: &LinkUniqueId) -> Option<ProcessorId>;
    ...
    fn outgoing_link_ids(&self, id: &ProcessorId) -> Vec<LinkUniqueId>;

    /// Get IDs of incoming links to a processor.
    fn incoming_link_ids(&self, id: &ProcessorId) -> Vec<LinkUniqueId>;
...
pub trait LinkQueryResult: Sized {
    fn from_ids(ids: Vec<LinkUniqueId>, graph: &impl GraphQueryInterface) -> Self;
    fn from_terminal(
        terminal: LinkTerminal,
        ids: Vec<LinkUniqueId>,
        graph: &impl GraphQueryInterface,
    ) -> Self;
}
...
impl LinkQueryResult for Vec<LinkUniqueId> {
    fn from_ids(ids: Vec<LinkUniqueId>, _graph: &impl GraphQueryInterface) -> Self {
        ids
    }
...
impl LinkQueryResult for Option<LinkUniqueId> {
    fn from_ids(ids: Vec<LinkUniqueId>, _graph: &impl GraphQueryInterface) -> Self {
        ids.into_iter().next()
    }
...
pub fn execute_link_query_builder(
    ...
    // Get starting set of IDs
    let mut ids: HashSet<LinkUniqueId> = match &builder.start {
...
pub fn execute_link_query_full<T>(
    ...
    // Get starting set of IDs
    let mut ids: HashSet<LinkUniqueId> = match &query.start {
        LinkStart::All => graph.all_link_ids().into_iter().collect(),
        LinkStart::FromProcessors { query, direction } => {
            let processor_ids = execute_processor_query_builder(query, graph);
            processor_ids
...
fn apply_link_step(
    step: &LinkStep,
    ids: HashSet<LinkUniqueId>,
    graph: &impl GraphQueryInterface,
) -> HashSet<LinkUniqueId> {
    match step {
