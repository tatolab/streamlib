...
use crate::core::link_channel::{LinkId, LinkPortAddress, LinkPortType};

use super::{BoxedProcessor, SimpleExecutor};

/// Wire a link by ID from the graph.
pub(super) fn wire_link(executor: &mut SimpleExecutor, link_id: &LinkId) -> Result<()> {
    ...
    wire_link_ports(executor, &from_port, &to_port)?;
    Ok(())
}

/// Unwire a link by ID.
#[allow(dead_code)]
pub(super) fn unwire_link(executor: &mut SimpleExecutor, link_id: &LinkId) -> Result<()> {
...
pub(crate) fn parse_port_address(port: &str) -> Result<(String, String)> {
...
fn wire_link_ports(
    ...
    let link = get_link_from_graph(executor, from_port, to_port)?;
    ...
    let (source_processor, dest_processor) =
        get_processor_pair(executor, &source_proc_id, &dest_proc_id)?;
    ...
    let exec_graph = executor.exec_graph_mut()?;
    exec_graph.insert_link_runtime(link_id.clone(), wired);
...
fn get_processor_pair(
    ...
    let exec_graph = executor.exec_graph_ref()?;
...
fn create_link_channel(
    ...
    match port_type {
        LinkPortType::Audio => {
            let (producer, consumer) = executor.link_channel.create_channel::<AudioFrame>(
                source_addr.clone(),
                dest_addr.clone(),
                capacity,
            )?;
        ...
        LinkPortType::Video => {
            let (producer, consumer) = executor.link_channel.create_channel::<VideoFrame>(
                source_addr.clone(),
                dest_addr.clone(),
                capacity,
            )?;
        ...
        LinkPortType::Data => {
            let (producer, consumer) = executor.link_channel.create_channel::<DataFrame>(
                source_addr.clone(),
                dest_addr.clone(),
                capacity,
            )?;
