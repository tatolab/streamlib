...
//! H.264 encode/decode example using the BGRA → compute shader → NV12 path.
//!
...
//! result as a Telegram-compliant MP4 (H.264 video + silent AAC audio).
//!
//! Prerequisites:
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // --- 2. Encode ---
    println!("=== H.264 ENCODE (BGRA → compute shader → NV12 → encode) ===");
    ...
    let output_path = output_dir.join(format!("h264_{timestamp}.mp4"));
    let raw_h264_path = std::env::temp_dir().join("h264_encoded.h264");

    {
        let mut h264_file = fs::File::create(&raw_h264_path)?;
        h264_file.write_all(&bitstream)?;
    }
    ...
    let _ = fs::remove_file(&raw_h264_path);
