...
/// (`<root>/tools/streamlib-cli` → `<root>`). It is a genuine streamlib
/// checkout, so checkout-facing paths exercise real behavior offline.
fn workspace_checkout() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .ancestors()
        .nth(2)
        .expect("workspace root above tools/streamlib-cli")
        .to_path_buf()
}
...
fn fake_crate_set() -> BTreeMap<String, PathBuf> {
    ...
    m.insert(
        "streamlib".to_string(),
        PathBuf::from("/checkout/sdk/streamlib-sdk"),
    );
    m.insert(
        "streamlib-idents".to_string(),
        PathBuf::from("/checkout/sdk/streamlib-idents"),
    );
    m
}
...
fn link_then_unlink_is_byte_clean_and_idempotent_across_cycles() {
    ...
    for cycle in 0..2 {
        ...
        assert_eq!(
            ...
            PathBuf::from("/some/checkout/sdk/streamlib-python"),
        ...
        assert!(
            std::fs::read_to_string(&deno)
                .unwrap()
                .contains("sdk/streamlib-deno/mod.ts")
        );
...
fn derive_linkable_crates_selects_the_streamlib_sdk_closure() {
    ...
    assert!(
        sdk.ends_with("sdk/streamlib-sdk"),
        "streamlib SDK member dir should be sdk/streamlib-sdk, got {}",
        sdk.display()
    );
...
fn real_link_offline_e2e_link_refresh_unlink_roundtrip() {
    ...
    assert!(
        linked.contains("sdk/streamlib-sdk"),
