...
# Every arm here constructs a real runtime, so none of them may go looking for
# the machine's actual mesh.
NO_DISCOVERY = {"mesh_multicast_discovery": False}
...
def test_a_mesh_name_outside_the_chunk_grammar_is_refused_naming_what_is_wrong():
    for outside_the_grammar in ["Lab", "9lab", "lab/two", "lab two", "@lab", ""]:
        with pytest.raises(RuntimeError) as refusal:
            streamlib.Runtime(mesh_name=outside_the_grammar, **NO_DISCOVERY)
...
def test_an_endpoint_this_build_cannot_open_is_refused_at_construction():
    for endpoint, what_the_refusal_names in [
        ...
        for door in ["mesh_peer_endpoints", "mesh_listen_endpoints"]:
            with pytest.raises(RuntimeError) as refusal:
                streamlib.Runtime(**{door: [endpoint]}, **NO_DISCOVERY)
            assert what_the_refusal_names in str(refusal.value), (
                f"the refusal of {endpoint!r} on {door} must name "
                f"{what_the_refusal_names!r}: {refusal.value}"
            )
...
def test_an_unreachable_peer_never_fails_the_runtime():
    ...
    runtime = streamlib.Runtime(
        mesh_peer_endpoints=["tcp/127.0.0.1:1"], mesh_multicast_discovery=False
    )
    runtime.shutdown()
...
def test_the_environment_carries_a_mesh_name_when_the_constructor_does_not(monkeypatch):
    ...
    with pytest.raises(RuntimeError) as refusal:
        streamlib.Runtime(**NO_DISCOVERY)
...
def test_the_constructors_mesh_name_outranks_the_environments(monkeypatch):
    ...
    runtime = streamlib.Runtime(mesh_name="lab", **NO_DISCOVERY)
    runtime.shutdown()
