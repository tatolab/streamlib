...
"""A playback `device_id` no backend on this machine can open.
...
different device — playing into the wrong speaker is worse than not playing.
Watching from a second thread is what the readiness wait is for: `run()` owns
