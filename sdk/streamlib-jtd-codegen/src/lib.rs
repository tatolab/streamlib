...
/// backends: digit-boundary lowercasing (`Test4D` → `Test4d`), Python-only
...
/// - Adds #[serde(deny_unknown_fields)] to structs — except `*Config` schemas,
///   which stay tolerant (config-as-bag: an unknown config key is ignored)
/// - Rejects an empty ("any value") schema instead of lowering it to an
///   untyped `Option<Value>` alias
...
fn post_process_rust(
    ...
    // Empty-schema degradation guard. jtd-codegen lowers a schema that declares
    // no shape (no `properties` / `optionalProperties` / `elements` / `enum` /
    // `discriminator` / `ref`) to `pub type <Root> = Option<Value>` — the
    // "any value" footgun that silently makes a typed config an untyped blob.
    // That path is deleted, not deprecated: a schema with no fields is an
    // authoring error. A processor that takes no config drops its `config:`
    // block (falling to `EmptyConfig`); a dynamic config declares
    // `type Config = Bag`.
    let type_alias_prefix = format!("pub type {ROOT_NAME_SENTINEL}");
    if code
        .lines()
        .any(|line| line.trim_start().starts_with(&type_alias_prefix))
    {
        anyhow::bail!(
            "schema `{expected_struct_name}` declares no fields, so jtd-codegen lowers it to an \
             untyped `Option<Value>` alias. This degradation is not allowed: declare at least one \
             property, or — for a config that takes no knobs — drop the processor's `config:` \
             block (it falls to `EmptyConfig`); for a fully dynamic config, use `type Config = Bag`."
        );
    }

    // Config structs get tolerant deserialize: `deny_unknown_fields` is banned
    // on the `*Config` type tree so an unknown/forward-compat config key is
    // ignored rather than rejected (config-as-bag: the typed view is sugar over
    // a tolerant bag). Data-frame and escalate schemas stay strict.
    let is_config_schema = expected_struct_name.ends_with("Config");
    ...
    // The Rust struct name we're currently emitting fields for, after rename.
    // Tracked so `#[serde(with = "serde_bytes")]` injection only fires on the
    // root struct's fields — never on inlined sub-structs (JTD `definitions:`
    // emit) or discriminator-variant payload structs that might happen to
    // share a field name with the root's binary field.
    ...
    for (idx, line) in post_lines.iter().enumerate() {
        ...
        if pending_decl_kind == Some(DeclKind::Struct) && line.starts_with("pub struct ") {
            ...
            if is_empty_inline {
                ...
                result.push_str(&format!("pub struct {} {{\n", short_name));
                in_struct = true;
                current_struct_name = Some(short_name.to_string());
            }
            in_enum = false;
            ...
            && !line.trim().starts_with("//")
        {
            ...
            //
            // Scoped to the root struct only: `binary_field_names` is
            // collected from the root schema's top-level `properties` /
            // `optionalProperties`, so applying it to a same-named field
            // in an inlined sub-struct (JTD `definitions:` emit) or
            // discriminator-variant payload struct would mis-tag a
            // non-binary field. The root struct is the one named
            // `ROOT_NAME_SENTINEL` at this point in the pipeline — the
            // final `result.replace(ROOT_NAME_SENTINEL, ...)` substitution
            // hasn't run yet, and sub-structs are always renamed off the
            // sentinel prefix.
            let is_root_struct = current_struct_name.as_deref() == Some(ROOT_NAME_SENTINEL);
            if is_root_struct {
...
/// PascalCase or `Fixture42Test`-style identifier → snake_case.
///
/// Rules:
/// - Insert `_` before each uppercase that follows a lowercase or after a
///   digit (so `VideoFrame` → `video_frame`, `Fixture42Test` → `fixture42_test`).
...
mod tests {
    ...
    fn post_process_python_substitutes_root_sentinel() {
        ...
        let out = post_process_python(code, "JtdCodegenFixtureA", None);
        assert!(out.contains("class JtdCodegenFixtureA:"));
        assert!(out.contains("-> 'JtdCodegenFixtureA':"));
    ...
    fn post_process_typescript_substitutes_root_sentinel() {
        ...
        let out = post_process_typescript(code, "JtdCodegenFixtureA");
        assert!(out.contains("export interface JtdCodegenFixtureA {"));
        assert!(!out.contains("StreamlibCanonRoot"));
    }
    ...
    #[test]
    fn post_process_rust_bans_deny_unknown_fields_on_config_schemas() {
        // Config-as-bag: a `*Config` type stays tolerant so an unknown /
        // forward-compat config key is ignored, not rejected. Mentally revert
        // the `is_config_schema` guard and this assertion flips.
        let code = "// Code generated by jtd-codegen for Rust v0.4.1\n\nuse serde::{Deserialize, Serialize};\n\n#[derive(Serialize, Deserialize)]\npub struct StreamlibCanonRoot {\n    pub gain: u32,\n}\n";
        let config_out = post_process_rust(code, "AudioMixerConfig", &[]).unwrap();
        assert!(
            !config_out.contains("deny_unknown_fields"),
            "config schema must not carry deny_unknown_fields, got:\n{}",
            config_out
        );

        // A non-config (data-frame / escalate) schema stays strict.
        let frame_out = post_process_rust(code, "VideoFrame", &[]).unwrap();
        assert!(
            frame_out.contains("#[serde(deny_unknown_fields)]"),
            "non-config schema must stay strict, got:\n{}",
            frame_out
        );
    }

    #[test]
    fn post_process_rust_escalate_schemas_stay_strict_config_suffix_drops_strict() {
        // Design §11 #7 landmine: `is_config_schema` keys tolerant-vs-strict
        // deserialize off the `Config` name suffix alone. Escalate is a typed
        // wire contract — `EscalateRequest` / `EscalateResponse` MUST keep
        // `deny_unknown_fields`, and only a `*Config` type may drop it. This
        // locks the suffix contract so a future non-config type named `*Config`
        // can't silently lose strictness. Mentally revert the `is_config_schema`
        // guard and the escalate assertions still pass but the config one flips —
        // revert the suffix check to a broad "always tolerant" and the escalate
        // assertions fire.
        let code = "// Code generated by jtd-codegen for Rust v0.4.1\n\nuse serde::{Deserialize, Serialize};\n\n#[derive(Serialize, Deserialize)]\npub struct StreamlibCanonRoot {\n    pub op: String,\n}\n";
        for escalate_type in ["EscalateRequest", "EscalateResponse"] {
            let out = post_process_rust(code, escalate_type, &[]).unwrap();
            assert!(
                out.contains("#[serde(deny_unknown_fields)]"),
                "escalate schema `{escalate_type}` must stay strict (typed wire \
                 contract), got:\n{out}"
            );
        }

        // A `*Config` type — and only a `*Config` type — drops strictness.
        let config_out = post_process_rust(code, "EscalateRequestConfig", &[]).unwrap();
        assert!(
            !config_out.contains("deny_unknown_fields"),
            "a `*Config` schema must drop deny_unknown_fields, got:\n{config_out}"
        );
    }

    #[test]
    fn post_process_rust_rejects_empty_any_value_schema() {
        // jtd-codegen lowers a fieldless schema to `pub type X = Option<Value>`
        // (the "any value" footgun). That path is deleted, not deprecated:
        // post-processing bails with a message pointing at EmptyConfig / Bag.
        let code =
            "// Code generated by jtd-codegen for Rust v0.2.1\n\nuse serde_json::Value;\n\npub type StreamlibCanonRoot = Option<Value>;\n";
        let err = post_process_rust(code, "FooConfig", &[]).unwrap_err();
        let msg = format!("{err:#}");
        assert!(
            msg.contains("EmptyConfig") && msg.contains("Bag"),
            "empty-schema error must point at EmptyConfig / Bag, got: {msg}"
        );
    }
    ...
    #[test]
    fn post_process_rust_only_emits_serde_bytes_on_root_struct() {
        // Two structs in the generated file (root + an inlined sub-struct from
        // a JTD `definitions:` block). Both have a field named `data` — the
        // root's is `Vec<u8>` and binary; the inlined one's would be a string
        // or non-binary value in a real schema. Only the root's `data` field
        // should receive the attribute, because `binary_field_names` was
        // collected from the root schema only.
        let code = "// Code generated by jtd-codegen for Rust v0.4.1\n\nuse serde::{Deserialize, Serialize};\n\n#[derive(Serialize, Deserialize)]\npub struct StreamlibCanonRoot {\n    pub data: Vec<u8>,\n    pub inner: InnerType,\n}\n\n#[derive(Serialize, Deserialize)]\npub struct InnerType {\n    pub data: String,\n}\n";
        let binary_fields = vec!["data".to_string()];
        let out = post_process_rust(code, "MyFrame", &binary_fields).unwrap();
        // Root struct's `data` gets the attribute.
        assert!(
            out.contains("    #[serde(with = \"serde_bytes\")]\n    pub data: Vec<u8>,"),
            "expected serde_bytes on root struct's `data: Vec<u8>` field, got:\n{}",
            out
        );
        // Inner struct's `data: String` MUST NOT get the attribute — applying
        // `with = "serde_bytes"` to a String would corrupt serialization.
        let attr_count = out.matches("#[serde(with = \"serde_bytes\")]").count();
        assert_eq!(
            attr_count, 1,
            "expected exactly 1 serde_bytes attribute (root struct only), found {}; out:\n{}",
            attr_count, out
        );
    }
    ...
    fn schema_name_to_module_name_strips_version_suffix() {
        assert_eq!(
            schema_name_to_module_name("com.tatolab.jtd_codegen_fixture_a.config@1.0.0"),
            "com_tatolab_jtd_codegen_fixture_a_config"
        );
    }
    ...
    fn pascal_to_snake_handles_digit_to_letter_boundary() {
        // Fixture42Test: digit 2 then capital T → underscore inserted.
        assert_eq!(pascal_to_snake("Fixture42Test"), "fixture42_test");
    ...
    fn classify_schema_legacy_shape_no_package_required() {
        let yaml = "metadata:\n  name: com.tatolab.jtd_codegen_fixture_a.config\nproperties: {}\n";
        let id = classify_schema(yaml, None).unwrap();
        assert_eq!(id.module_name, "com_tatolab_jtd_codegen_fixture_a_config");
        assert_eq!(id.struct_name, "JtdCodegenFixtureAConfig");
    ...
    fn schema_identity_output_path_flat() {
        let id = SchemaIdentity {
            module_name: "com_tatolab_jtd_codegen_fixture_a_config".to_string(),
            struct_name: "JtdCodegenFixtureAConfig".to_string(),
            package_subdir: None,
        };
        let p = id.output_path(Path::new("/out"), "rs");
        assert_eq!(
    ...
    fn to_pascal_case_with_digits() {
        assert_eq!(to_pascal_case("fixture42_test"), "Fixture42Test");
    }
    ...
    fn group_entries_separates_flat_and_per_package() {
        ...
        assert_eq!(flat[0].struct_name, "JtdCodegenFixtureAConfig");
    ...
    fn write_typescript_barrels_re_exports_subpackage_index() {
        ...
        assert!(top.contains("export * from \"./com_tatolab_jtd_codegen_fixture_a_config.ts\";"));
