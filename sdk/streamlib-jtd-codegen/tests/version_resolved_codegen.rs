...
//! End-to-end: a package-source-cached crate's codegen resolves a schema
//! dependency from a (hermetic `file://`) static generic store and
//! generates bindings — the exact pipeline `build.rs` drives, minus the
//! cargo build.
//!
//! This is the faithful E2E for issue #1116's "a package-source-cached
...
use streamlib_idents::{PackageSource, ResolverOptions, resolve_with};
...
/// laid out the way the `file://` package source transport expects (base URL is the
...
#[test]
fn version_resolved_schema_codegen_emits_imported_type() {
    let test_name = "version_resolved_schema_codegen_emits_imported_type";
    ...
    // Three versions in the package source; the ^1.0.0 range must select 1.2.0.
    ...
    // Consumer manifest: bare version range + External schema import, NO
...
"#,
    ...
    .expect("by-version resolution must succeed without a path patch");

    // Highest-in-range version selected from the package source.
    let escalate = resolved
        .packages
        .get("@tatolab/escalate")
        .expect("escalate resolved from the package source");
    assert_eq!(
        escalate
            .manifest
            .package
    ...
    generate_from_resolved(&resolved, RuntimeTarget::Rust, output.path())
        .expect("codegen from version-resolved packages");
    ...
    assert!(
        ...
        "expected escalate_request.rs under tatolab__escalate/ (version-resolved External owner context); got files: {files:?}"
    );
}
...
/// ships — a bare version range in `dependencies:` PLUS a monorepo-relative
...
/// fall back to resolving the declared version from the package source and codegen
...
#[test]
fn version_resolved_codegen_falls_back_when_dev_path_patch_absent() {
    let test_name = "version_resolved_codegen_falls_back_when_dev_path_patch_absent";
    if skip_unless_jtd_codegen_available(test_name) {
        return;
    }
...
"#,
    ...
    .expect("absent dev path patch must fall back to by-version resolution");

    let escalate = resolved
        .packages
        .get("@tatolab/escalate")
        .expect("escalate resolved from the package source despite the path patch");
    assert_eq!(
        escalate
            .manifest
            .package
