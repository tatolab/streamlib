...
struct PythonNativeContextInner {
    ...
    /// feeds. A processor consuming N channels holds N entries; routing is by the
    /// binding, not the frame's stamped port key (channel-centric transport,
    /// #1419).
    ...
    /// All inputs share this listener (the notify service stays destination-keyed
    /// so a processor waits on one fd regardless of fan-in).
    notify_listener: Option<Listener<ipc::Service>>,
}

struct SubscriberState {
    /// Local input port this channel subscriber feeds — the receive path routes
    /// every frame this subscriber delivers into this port.
    local_port: String,
...
struct PublisherState {
    ...
    /// only; destinations route by their subscriber binding, not this key.
    source_port: String,
...
/// input port it feeds.
///
/// `channel_service_name` is the SOURCE channel (`{source}/{source_port}`);
/// `local_port` is this subscriber's destination input port (the receive path
/// routes every frame this subscriber delivers into `local_port`). `max_subscribers`
/// is the channel's destination fan-out plus the reserved tap slot — it must match
/// what the host and every other opener request (iceoryx2 verifies it on `open`).
...
pub unsafe extern "C" fn slpn_input_poll(ctx: *mut PythonNativeContext) -> i32 {
    ...
    // (a channel's single publisher stamps its own source port).
...
///
/// `channel_service_name` is the SOURCE channel (`{source}/{source_port}`) — the
/// port's single publisher is created once and reused; subsequent calls for the
/// same `port_name` only append another destination notifier. `max_subscribers`
/// is the channel's destination fan-out plus the reserved tap slot;
/// `notify_max_notifiers` is the DESTINATION's fan-in — both must match the host
/// (iceoryx2 verifies them on `open`, so a mismatch breaks cross-language wiring).
...
pub unsafe extern "C" fn slpn_output_write(
    ...
    // Returns SLPN_WRITE_REFUSED_OVER_CEILING so the SDK drops without raising.
...
/// only ever has one destination-keyed Event service. `notify_max_notifiers` is
/// the destination's fan-in and must match the host's (iceoryx2 verifies it on
...
mod gpu_surface {
    ...
    impl Drop for SurfaceHandle {
        fn drop(&mut self) {
            ...
            // owns that dup. Dropping the `ConsumerVulkanBuffer`
...
mod cpu_readback {
    ...
    /// DMA-BUF FD as its own [`ConsumerVulkanBuffer`], imports the
    ...
    pub unsafe extern "C" fn slpn_cpu_readback_register_surface(
        ...
        // Import each plane as its own ConsumerVulkanBuffer.
