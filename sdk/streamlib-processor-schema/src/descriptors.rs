...
//!
//! These are engine-free serde data types shared by the engine, the
//! `#[processor]` macro, and the plugin-SDK. `ProcessorDescriptor` crosses
//! the cdylib plugin ABI as `rmp_serde` msgpack (host
//! `host_processor_register` / cdylib `register_via_callback`), so a single
//! definition here keeps both sides byte-identical.

use serde::{Deserialize, Serialize};

use crate::{Org, Package, PortSchemaSpec, ProcessorScheduling, SchemaIdent, SemVer, TypeName};

/// Lossless wire-format mirror of [`PortSchemaSpec`] used at the cdylib
/// plugin ABI msgpack boundary.
///
/// `PortSchemaSpec`'s default serde impl is intentionally YAML-shaped —
/// `Specific(SchemaIdent)` serializes to just the bare type-name string, and
/// the matching `Deserialize` impl only ever produces `Any | Named`. That
/// contract holds for `streamlib.yaml` authoring, where the bare PascalCase
/// form is canonical and the downstream resolver (`ProjectConfig` /
/// the `#[streamlib::sdk::processor]` proc-macro) rewrites `Named → Specific`
/// against the enclosing manifest's `schemas:` map.
///
/// It does NOT hold when [`ProcessorDescriptor`] crosses the cdylib dlopen
/// plugin ABI seam via `rmp_serde`. The proc-macro pre-resolves
/// `Named → Specific` at the cdylib's compile time and embeds the
/// fully-qualified [`SchemaIdent`] in the cdylib's `descriptor()` constant.
/// When the host deserializes the msgpack envelope, the default YAML-shaped
/// impl would downcast the carried `Specific` to `Named(bare type-name)` —
/// the cdylib's resolution work silently lost. The wire-only mirror below
/// restores the structured-everywhere contract: every reference to a schema
/// identifier is a structured record on every wire surface. Used via
/// `#[serde(with = "port_schema_spec_wire")]` on [`PortDescriptor::schema`] —
/// every `ProcessorDescriptor` that crosses a serializer boundary now
/// round-trips losslessly.
///
/// YAML authoring (`streamlib.yaml`) continues to use `PortSchemaSpec`'s
/// default impl — `schema: VideoFrame` parses as `Named(VideoFrame)`, then
/// the manifest-side resolver rewrites it to `Specific(SchemaIdent)` before
/// the descriptor is built. The wire mirror only governs the
/// `ProcessorDescriptor` msgpack/JSON hop downstream of that resolution.
pub mod port_schema_spec_wire {
...
mod tests {
    ...
    fn port_descriptor_schema_msgpack_round_trip_preserves_any() {
        let pd = PortDescriptor::new("frames", "Wildcard frames", PortSchemaSpec::Any, false);
        let bytes = rmp_serde::to_vec_named(&pd).expect("encode");
        let pd2: PortDescriptor = rmp_serde::from_slice(&bytes).expect("decode");
