...
export const meta = {
  ...
  phases: [
    ...
    {
      name: 'Test',
      description: 'local-ci-runner runs the gate battery in the worktree and returns a pass/fail table.',
    },
    {
      name: 'SelfReview',
      description:
...
let rederive = { plan_of_record: '', shape_confirmed: shape };
let ciTable = {};
let report = { tests: {}, needs_bench: false, deviations: [], followup_candidates: [] };

phase('Rederive', async () => {
  rederive = await agent(
  ...
  log(`rederive posted=${rederive.posted === true} shape=${rederive.shape_confirmed}`);
});

phase('Implement', async () => {
  const bugFirst =
...
phase('SelfReview', async () => {
  const finalReport = await agent(
    `Self-review the diff for issue #${issue} against its plan-of-record and the local gate results below. ` +
      ...
      `Implement report: ${JSON.stringify(report)}\nLocal gates: ${JSON.stringify(ciTable)}`,
