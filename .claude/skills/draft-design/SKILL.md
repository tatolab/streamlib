...
Spawn the domain experts whose zones the design touches (gpu-vulkan-expert, linux-media-expert, plugin-abi-expert, polyglot-ipc-expert, package-source-expert) **in parallel**, read-only, each on a specific recon question: what core system already covers this concern, what would have to change, what the contract invariants and known failure modes are. The experts re-derive from the tree and cite `file:line` — the brief stands on evidence, not memory. Spawn the experts yourself, in one message so they run concurrently.
...
### 3. Draft the brief for the owner
...
- **DECISIONS FOR OWNER** — an explicit, numbered list of the calls only the owner can make (a trade-off with no clear winner, a scope boundary, a milestone question). This is the point of the brief.
...
Nothing builds until the owner approves in a comment. Under the loop, this is a park (the ticket waits on their answer). Interactively, surface the DECISIONS-FOR-OWNER list to them directly. Either way, the brief is the deliverable — do not start implementing off an unapproved design.
