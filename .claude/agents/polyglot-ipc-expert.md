...
- **The capability boundary is the process, plus the exchange client.** A helper's GPU contexts are backed by the parent round trip and the surface-share socket; with neither of those wired the GPU calls refuse by name rather than reaching a privileged primitive locally. Never hand a helper a privileged path that bypasses the bridge.
