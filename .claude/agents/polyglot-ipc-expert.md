...
- **One Python processor, one helper process, one GIL.** Helper-process placement (engine exec of `sys.executable`, never fork) is the only placement; hosting a processor in the app's interpreter is a STOP-WORK violation (`.claude/rules/placement.md`). Vocabulary note: "subprocess" elsewhere in this file predates the pivot and means the retired cdylib adapter runtime — helper placement is the surviving, load-bearing sense.
