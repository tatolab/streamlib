...
- **No raw-`Arc` transit crosses the ABI — the slots were deleted (#1270).** The FullAccess `host_vulkan_device_arc` / `host_vulkan_texture_arc` / raw-timeline slots that transited non-`#[repr(C)]` host types are gone; a plugin cannot even name the transit types, so the cross-build silent-corruption mode is closed by construction (not by a runtime guard). The load handshake is `abi_version` (read first) + `abi_layout_fingerprint`; the #1247 transit-fingerprint was removed along with the transit it guarded. Every FullAccess resource crosses as a PluginAbiObject or a msgpack byte buffer — there is no raw transit left to make safe, and no lift to do.
