...
- **A clean run is NO evidence that raw-device construction is sound.** GPU code that hand-rolls a Vulkan object on a raw device instead of going through `GpuContext` can run clean for a long time and still corrupt the driver — the failure is latent, not immediate. Never treat "it worked" as proof the construction path is legitimate; check the gateway rule instead.
