...
- The package source (the static `.slpkg` file tree: store + catalog + release manifest).
- The runtime module-loader resolution (by-version, by-URL, by-path).

## Method — how you work
- **Distribute by version, never by path or git-patch.** Every StreamLib-authored or customized artifact resolves by version in anything a consumer sees; a relative `path` or a git `[patch]` is a dev-loop-only affordance. For local development use the sanctioned link loop: whole-tree link against a checkout (`streamlib link --engine`, materialized as `[patch.crates-io]` path overrides) — the dev loop has no publish step. Never introduce a new persistent path/patch cross-crate dep in a manifest.
...
- **Engine/SDK crate deps in packages declare a version but resolve to the local checkout — there is no central package registry and NO publish step.** A package's `Cargo.toml` pins e.g. `version = "0.6.0"` for the engine/SDK crates, but that version resolves to the local workspace via `streamlib link --engine` (`[patch.crates-io]` path overrides), not a fetch from crates.io or any hosted service — resolve by version from a package source, or `streamlib link` a checkout. A package conversion or engine/SDK dep bump is therefore NEVER "blocked on a republish"; treat any such conclusion as a misdiagnosis and re-verify against `docs/architecture/package-source.md` and `docs/architecture/package-development-model.md`.
...
- **A package source is any location serving versioned `.slpkg` files — a static file tree over `file://` or a dumb HTTP mount, tokenless to read.** There is no central/hosted registry; never reintroduce a hosted-daemon backend — a static tree fully covers `.slpkg` distribution.
...
The on-disk package-source tree layout, the exact `.slpkg` build/publish CLI verbs, the module-loader strategy enum, the catalog schema, the lockfile format, and the schema-codegen entry points all drift. Read `runtime/streamlib-idents`, the pack crate, the module loader, and the relevant `docs/architecture/` package-source docs at need and cite `file:line`. Treat any doc claim as the best-known state when written and confirm against the code before relying on it.
