...
- **Engine/SDK crate deps in packages declare a version but resolve to the local checkout — there is NO crates registry and NO publish step.** A package's `Cargo.toml` pins e.g. `version = "0.6.0"` for the engine/SDK crates, but that version resolves to the local workspace via `streamlib link --engine` (`[patch.crates-io]` path overrides), not a fetch from any registry — the crates registry was removed. A package conversion or engine/SDK dep bump is therefore NEVER "blocked on a republish"; treat any such conclusion as a misdiagnosis and re-verify against `docs/architecture/static-registry.md` and `docs/architecture/package-development-model.md`.
