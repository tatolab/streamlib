...
| HISTORICAL, closed by construction — a separately-built `.slpkg` cdylib transited a non-`repr(C)` host device and double-freed in `vkCreatePipelineLayout` / `libnvidia-gpucomp`. The transit and the cdylib boundary are deleted; read it only as the empirical why behind the RHI gateway rule, never as a live symptom | `docs/learnings/slpkg-raw-device-rhi-construction.md` |
