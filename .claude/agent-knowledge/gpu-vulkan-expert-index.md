...
| HISTORICAL, closed by construction — a separately-built plugin cdylib transited a non-`repr(C)` host device and double-freed in `vkCreatePipelineLayout` / `libnvidia-gpucomp`. The transit and the cdylib boundary are deleted; read it only as the empirical why behind the RHI gateway rule, never as a live symptom | `docs/learnings/slpkg-raw-device-rhi-construction.md` |
...
| You changed GPU-pipeline code and need to see the frames it actually produced — any channel, mid-graph included, with no window in the graph and no display server in the path | `.claude/skills/verify-live/SKILL.md` (the `exchange` door) |
| Running the camera→display fixture end-to-end without physical hardware, or confirming the present / swapchain path specifically — the one stretch a window capture is still the right tool for | `docs/learnings/camera-display-e2e-validation.md` |
