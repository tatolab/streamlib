...
| A helper process consuming an imported cross-process `VkImage` trips a layout VUID, or gets a discarded/black frame — layout is independent per `VkDevice`; coordinate via QFOT release/acquire or the bridging fallback, not a shared tracker | `docs/learnings/cross-process-vkimage-layout.md` |
| SIGSEGV when a second `VkDevice` is created with the first busy — the evidence behind "a helper stays consumer-only / one parent `VkDevice`" | `docs/learnings/nvidia-dual-vulkan-device-crash.md` |
| A design, diagnostic, or watchdog premised on Python processors sharing a GIL or an interpreter — or any in-process hosting proposal | `.claude/rules/placement.md` + `docs/decisions/helper-process-placement-only.md` |
